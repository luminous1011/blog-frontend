...
export const GITHUB_CHART_URL: string =
  "https://ghchart.rshah.org/luminous1011";
export const QQ_CONTACT_URL: string =
  "http://wpa.qq.com/msgrd?v=3&uin=1980393754&site=qq&menu=yes";
export const MAIL_CONTACT_URL: string =
  "http://mail.qq.com/cgi-bin/qm_share?t=qm_mailme&email=aRkDGhkIBykPBhEECAAFRwoGBA";
...
const COMMENT_BASE =
  "8664e4e4cb09d6954da82203d8fca900__vue3_typescript_remember_author_";
...
export function browserRedirect() {
  ...
  if (isWin) {
    const isWin2K =
      sUserAgent.indexOf(WIN_2K) > -1 || sUserAgent.indexOf(WINDOWS_2K) > -1;
    if (isWin2K) return WINDOWS_2K;
    const isWinXP =
      sUserAgent.indexOf(WIN_XP) > -1 || sUserAgent.indexOf(WINDOWS_XP) > -1;
    if (isWinXP) return WINDOWS_XP;
    const isWin2003 =
      sUserAgent.indexOf(WIN_2003) > -1 ||
      sUserAgent.indexOf(WINDOWS_2003) > -1;
    if (isWin2003) return WINDOWS_2003;
    const isWinVista =
      sUserAgent.indexOf(WIN_VISTA) > -1 ||
      sUserAgent.indexOf(WINDOWS_VISTA) > -1;
    if (isWinVista) return WINDOWS_VISTA;
    const isWin7 =
      sUserAgent.indexOf(WIND_7) > -1 || sUserAgent.indexOf(WINDOWS_7) > -1;
    if (isWin7) return WINDOWS_7;
    const isWin10 =
      sUserAgent.indexOf(WIND_10) > -1 || sUserAgent.indexOf(WINDOWS_10) > -1;
...
export function importModule(_module: Record<string, () => Promise<unknown>>) {
  const __moduleTemp = Object.keys(_module)[0];
  _module && __moduleTemp && _module[__moduleTemp]();
}
